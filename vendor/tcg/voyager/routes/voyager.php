...
//customization from here for create delivery system

    Route::group(['prefix'=>'delivery','name'=>'orders'],function()use ($namespacePrefix) {
      Route::get('orders', ['uses' => $namespacePrefix.'deliveryController@loadOrders'])->middleware('auth');

      Route::get('orders/deatils/{id}', ['uses' => $namespacePrefix.'deliveryController@OrderDetalis'])->middleware('auth');

      Route::get('orders/finishorder/{id}/{status}', ['uses' => $namespacePrefix.'deliveryController@FinishOrder'])->middleware('auth');

      Route::get('orders/loadmap/{longitude}/{latitude}', ['uses' => $namespacePrefix.'deliveryController@LoadMap'])->middleware('auth');

      Route::get('orders/map', ['uses' => $namespacePrefix.'deliveryController@orderMap'])->middleware('auth');
